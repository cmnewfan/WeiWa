...
class MainFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        (activity as MainActivity).setOnWeiboPOJOUpdated(object : onWeiboUpdatedListener {
            override fun onUpdate(pojo: WeiboPojo, updateType: Int) {
                if (mWeiboAdapter == null || updateType == 0) {
                    mWeiboAdapter = WeiboAdapter(pojo, this@MainFragment, onNeedInsert!!)
                    mRecyclerView!!.adapter = mWeiboAdapter
                    mWeiboAdapter!!.setData(pojo, true)
                } else if (updateType == 7 || updateType == 8) {
                    mWeiboAdapter!!.setData(pojo, false)
                } else {
                    if (pojo == null || pojo.statuses!!.size == 0) {
                        Toast.makeText(activity, "已更新至最后一条", Toast.LENGTH_SHORT).show()
                    } else {
                        mWeiboAdapter!!.insertNewData(pojo)
                    }
                }
                (activity as MainActivity).stopAnimation()
            }
            ...
            override fun onComment(comment: WeiboCommentPojo) {
                ...
                if (comment.status!!.retweeted_status != null && comment.status!!.retweeted_status!!.id != null) {
                    ...
                    (activity as MainActivity).setFragment(weiboRetweetCommentFragment)
                } else {
                    val weiboCommentFragment = WeiboCommentFragment()
                    weiboCommentFragment.arguments = bundle
                    (activity as MainActivity).setFragment(weiboCommentFragment)
                }
            }
        })
